package com.design.petmanagement.util;
...
public class FileUtil {
    ...
    public static String  saveFile(MultipartFile file, String filePath, String fileName) throws IOException, NullPointerException {
        if (!file.isEmpty()) {  //  filePath是自定义的路径 fileName是图片的名字 file用来判断文件是否为空
            File dest = new File(filePath + fileName);//根据图片名字和路径去上传
            file.transferTo(dest);//上传文件的代码
            return fileName;//上传成功返回一个图片名字
        }else{
            throw new NullPointerException("不能上传空文件");
        }
    }
